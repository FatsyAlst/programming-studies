# Learning context document

I'm a student and this is the learning context document I built to help us 
work together effectively. I want to use AI to enhance my learning, 
not replace it. Here's my context:...

## Academic Background

- **Current Program:** Electrical Engineering student (transitioning to Computer Science or Information Systems)
- **Target Program:** Computer Science (Bachelor's degree, starting next year) abroad or Information Systems (Bachelor's degree, through internal transfer at USP)
- **Location:** São Carlos, Brazil (studying at USP)
- **Languages:** Bilingual - Portuguese (native), English (intermediate, preparing for IELTS)
- **Academic Goal:** Apply to international Computer Science programs

**Focus Areas:**

Still choosing, still not sure what I'm going to specialize in (Data Science, DevOps or Devolpment - I have these in mind). At the moment, I'm still studying and getting a strong base:

- Python programming fundamentals
- Data structures and algorithms
- Testing (unittest, doctest)
- Software documentation (LaTeX, Markdown)
- File I/O and data manipulation
- Algorithm design and analysis

## Learning Style & Preferences

- **Practice over theory** - I learn by doing, building, and solving problem (taking courses and watching classes help me too, but exercising is the best for me)
- **Progressive difficulty** - I prefer exercises that build on previous knowledge and make me really think and train my critical analysis about the topics I studied (applying what I studied for other problems and fields, for example)
- **English immersion** - Keep exercises and explanations in English to maintain language practice
- **Hands-on projects** - I didn't do any project yet, but in order to get more visibility as a beginner in this new field, I have to start with some projects later. I hope real applications will help me understand concepts better and train as well.
- **Structured exercises** - Clear problem statements with examples (like course exercise dynamics)

**My Strengths:**

- Self-motivated and disciplined
- Good at technical documentation
- Comfortable with command-line tools
- Systematic approach to problem-solving

**My Challenges:**

- Sometimes need help transitioning from theory to practice
- Want to improve algorithmic thinking
- Need to balance multiple learning resources effectively

**Tools I Use:**

- VSCode (primary editor)
- Jupyter Notebooks 
- GitHub
- Terminal/Command line

**Currently Working In:**

- Programming studies repository (working on it and uploading it continuously)
- Coursera course exercises and mini-projects (courses and specializations)
- Practice exercises from various sources 

## How I Want to Work with AI

**DO Help Me With:**

- ✅ Generating practice exercises at moderate difficulty
- ✅ Explaining concepts when I'm stuck (but let me try first)
- ✅ Reviewing my code and suggesting improvements
- ✅ Creating study materials and summaries (.md files)
- ✅ Providing multiple approaches to solve problems
- ✅ Helping with documentation and README files
- ✅ Clarifying programming concepts

**DON'T Do For Me:**

- ❌ Write complete solutions without me trying first
- ❌ Do homework/assignments entirely
- ❌ Give me answers before I attempt the problem
- ❌ Skip explanations of *why* something works

**My Ideal Interaction:**

1. I explain what I'm trying to learn or build
2. You help me break it down into steps
3. I try to implement it
4. You review and guide improvements
5. We discuss the approach and alternatives

**When I'm Stuck:**

- Give me hints first, not full solutions
- Ask me guiding questions to help me think
- Point me to relevant concepts or documentation
- Only show code examples after I've attempted

## Academic Integrity & Boundaries

**My Commitments:**

- I will always attempt problems independently first
- I will be transparent about AI assistance in my learning
- I will not use AI to complete graded assignments without disclosure
- I will verify and understand everything before using it in my work
- I will cite AI assistance when required by instructors

## Always do!

- Network with students and professionals
- Stay current with industry trends and best practices
- Physical activity (gym and running, but focus on the gym!)

## Current Goals (Short-term)

- Complete Python specializations in Coursera
- Build a solid foundation in Python before transitioning to CS or IS program
- Practice consistently through exercises (target: almost daily practice)
- Prepare comprehensive study notes in Markdown while studying
- Set up GitHub repository with proper structure and documentation
- Continue IELTS preparation for international applications (target IELTS: 6.5-7.0)

## Medium-term goals

- Complete transition to Computer Science or Information Systems program
- Earn Coursera Specializations certificates on Python
- Achieve target IELTS score for international CS program applications
- Apply to international Computer Science programs
- Build strong foundation in Python
- Think of project ideas for beginners
- Enhance my GitHub profile
- Explore internship opportunities in software development

## Long-term goals

- Build 3-5 substantial personal projects demonstrating programming skills
- Master Python (intermediate to advanced level)
- Develop strong problem-solving skills through competitive programming platforms and exercises
- Develop technical writing and communication skills
- Contribute to open-source projects (when ready)
- Get jacked! (gym)

## My dreams for the future

- Build career in DevOps, software engineering, data science, or related field
- Live abroad

## Questions to Ask Me

When starting a session, feel free to ask:

- "What are you working on today?"
- "Have you tried solving this first?"
- "What part are you stuck on?"
- "What have you learned so far in this topic?"

## Notes & Reminders

- I prefer concise, direct explanations (not overly verbose)
- Code examples are very helpful
- I appreciate when you point out best practices
- Feel free to challenge my thinking
- Remind me to test my code and write good documentation when you see issues

---

**Last Updated:** November 27, 2025  
**Next Review:** When starting new semester/program
