...
def board_contains_word_in_column(board, word):
    ...
    for row_index in range(len(board)): # in case the board is not rectangular
        for column_index in range(len(board[row_index])):
            if word in make_str_from_column(board, column_index):
                return True
    return False

# print(board_contains_word_in_column([['A', 'N', 'T', 'T'], ['X', 'S', 'O', 'B']], 'NO'))
...
def word_score(word):
    ...
    elif 3 <= len(word) <= 6: return len(word)
    elif 7 <= len(word) <= 9: return 2 * len(word)
    else: return 3 * len(word)
...
def update_score(player_info, word):
    ...
    # return player_info[1] ## added this just to check if it was indeed updating the score
...
def num_words_on_board(board, words):
    ...
    count = 0
    for word in words:
        if board_contains_word(board, word): count += 1
